package com.emc.moodmingle.ui.create.dailymood.settings.timing
...
import com.emc.moodmingle.domain.remote.model.post.dailymood.settings.SettingsTiming
...
@Composable
fun TimingScreen(settings: DailyMoodSettings, onEdit: (DailyMoodSettings) -> Unit) {
    ...
    TimingOption(
        ...
        onClick = { onEdit(settings.copy(timing = SettingsTiming(type = TimingType.AUTO_POST_NOW))) }
    )
    ...
    TimingOption(
        ...
        onClick = { onEdit(settings.copy(timing = SettingsTiming(type = TimingType.MANUAL_ONLY))) }
    )

    // Show scheduler UI if selected
    if (timing.type == TimingType.SCHEDULE) {
        Spacer(modifier = Modifier.height(24.dp))
        ScheduleSection(timing) { selectedAction = it }
    }
    ...
    when (selectedAction) {
        "date" -> TimingDatePickerDialog(settings, onEdit) { selectedAction = "" }
        "time" -> TimingTimePickerDialog(settings, onEdit) { selectedAction = "" }
    }
}
