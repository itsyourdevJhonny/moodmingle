package com.emc.moodmingle.ui.create.dailymood.settings.reaction
...
@Composable
fun ReactionSettingsScreen(settings: DailyMoodSettings, onEdit: (DailyMoodSettings) -> Unit) {
    val enabled = settings.reactionEnabled

    LoveReactionCard(
        enabled = enabled,
        onToggle = { onEdit(settings.copy(reactionEnabled = it)) }
    )
