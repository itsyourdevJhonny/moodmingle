package com.emc.moodmingle.ui.dailymood.settings.sharing
...
import com.emc.moodmingle.data.firebase.model.post.dailymood.settings.SharingSettings
...
fun SharingForwardingScreen(
    ...
    Scaffold(
        ...
        topBar = { ScaffoldHeader(title = "Sharing & Forwarding") { onDismiss() } }
    ) { paddingValues ->
        Content(paddingValues, sharingSettings, onSettingsEdited, settings)
    }
}
        ...
        PermissionSectionCard {
            PermissionToggleRow(
                ...
                onCheckedChange = { value ->
                    onSettingsEdited(
                        settings.copy(sharingSettings = SharingSettings(allowExternalSharing = value))
                    )
                }
            )
            ...
            PermissionToggleRow(
                ...
                onCheckedChange = { value ->
                    onSettingsEdited(settings.copy(sharingSettings = SharingSettings(allowForwarding = value)))
                }
            )
        }
