...
class ShareRepositoryFirebase @Inject constructor(
    ...
    fun getSharedByPostId(postId: String): Flow<ShareEntityFirebase?> = callbackFlow {
        val listener = sharesCollection
            ...
            .addSnapshotListener { snapshot, error ->
                ...
                trySend(snapshot?.documents?.firstOrNull()?.toObject(ShareEntityFirebase::class.java))
            }
        awaitClose { listener.remove() }
    }
     ...
     */
    fun getSharedByPostIdAndUserUid(postId: String, userUid: String): Flow<ShareEntityFirebase?> = callbackFlow {
        val listener = sharesCollection
            .whereEqualTo("postId", postId)
            .whereEqualTo("userUid", userUid)
            .addSnapshotListener { snapshot, error ->
