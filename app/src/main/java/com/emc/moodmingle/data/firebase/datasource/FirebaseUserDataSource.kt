...
class FirebaseUserDataSource(
    ...
    suspend fun loginUser(email: String, password: String): Result<UserEntityFirebase> = runCatching {
        val result = auth.signInWithEmailAndPassword(email, password).await()
        val uid = result.user?.uid ?: throw Exception("Login failed: no UID")
        val snapshot = usersRef.document(uid).get().await()
        snapshot.toObject(UserEntityFirebase::class.java) ?: throw Exception("User not found in Firestore")
    }
    ...
    fun getAllUsers(): Flow<List<UserEntityFirebase>> = callbackFlow {
        val listener = usersRef.addSnapshotListener { snapshot, error ->
            if (error != null) {
                ...
                val users = snapshot.documents.mapNotNull { it.toObject(UserEntityFirebase::class.java) }
    ...
    fun searchUsersByUsername(usernameQuery: String): Flow<List<UserEntityFirebase>> = callbackFlow {
