...
fun ShowProfilePicture(avatarUrl: String, onDismiss: () -> Unit) {
    ...
    Dialog(onDismissRequest = onDismiss, properties = dialogFullSizeProperties()) {
        Box(
            modifier = Modifier
                .fillMaxSize()
            ...
            BackIcon(onDismiss)
            DownloadIcon(context, avatarUrl)
        }
    }
}

@Composable
private fun BoxScope.BackIcon(onDismiss: () -> Unit) {
    Icon(
...
@Composable
private fun BoxScope.DownloadIcon(context: Context, avatarUrl: String) {
